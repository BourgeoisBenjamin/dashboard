...
router.put('/youtube/statistics-video/:id_widget', JWTService.authenticateToken, function (req, res) {
    pool.getPool().query("UPDATE statistics_video_youtube SET activate = $3, id_video = $4 WHERE id = $1 AND id_youtube_service IN (SELECT id FROM youtube_service WHERE id_user = $2) RETURNING id", [req.params.id_widget, req.user.user_id, req.query.activated, req.query.video_id], (err, result) => {
        if (err) {
            res.status(503);
            res.json({message: "Service Unavailable"})
        } else {
